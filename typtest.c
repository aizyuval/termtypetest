...
void wrap_insch(WINDOW *win, int existing_space[], int line_number,
                int text_line_length, unsigned int *awaiting_words,
                char recvChar, int in_x, int in_y);
void wrap_delch(WINDOW *win, int existing_space[], int line_number,
                int text_line_length, unsigned int *awaiting_words,
                struct _strings *strings, int in_x);

struct stats_result *spawn_test(WINDOW *testwin,
                                struct stats_file **address_to_stats,
                                struct stats_file *stats, int *recvChar);
void spawn_stats(WINDOW *statswin, struct stats_result *statisticsP,
                 struct stats_file *stats, int *recvChar);

struct _word *init_text(struct _word *initializer_ll, unsigned int *wordAmount,
                        struct _strings *strings, int line_number,
                        int *existing_space, WINDOW *textWin,
                        unsigned int *awaiting_words);

struct _strings *
start_keys(struct stats_file **address_to_stats, struct stats_file *stats,
           int *recvChar, unsigned *time_cyc, unsigned timeopts,
           long *time_out_num, long time_out[], WINDOW *timeWin,
           char **languages, unsigned *lang_cyc, unsigned old_lang_cyc,
           unsigned lang_amount, signed lang_diff, WINDOW *langWin,
           struct _word *printed_ll, unsigned int *wordAmount,
           struct _strings *strings, int line_number, int existing_space[],
           int text_line_length, WINDOW *testwin, WINDOW *textWin,
           unsigned int *awaiting_words);

struct _strings *
restart_text(WINDOW *timeWin, WINDOW *langWin, long time_out_num,
             char **languages, unsigned time_cyc, struct _word *linked_ll,
             unsigned int *wordAmount, struct _strings *strings,
             int line_number, int *existing_space, int text_line_length,
             WINDOW *textWin, unsigned int *awaiting_words, char *lang);
...
int main() {
  ...
  while ((*recvChar) != KEY_ENT) {
    ...
    if (homedi == NULL) {
      printw("couldnt get your home dir");
    }
    ...
    cyaml_err_t stats_err =
        cyaml_load_file(homedir, &config, &stats_file_schema_top,
                        (cyaml_data_t **)&stats, NULL);
...
struct _word *init_text(struct _word *initializer_ll, unsigned int *wordAmount,
  ...
  for (size_t i = 0; i < (*wordAmount);
       initializer_ll = initializer_ll->next_word) {
    ...
    if (!outofwords) {
      ...
      if (existing_space[line_number] >
          length) { // only > because if equal we can't fill it. need the user
                    // to space to move a line
    ...
    memset(initializer_ll->ver_arr, NONTYPED_CHAR,
           strlen(strings->words_member[i]));
...
start_keys(struct stats_file **address_to_stats, struct stats_file *stats,
  ...
  while ((*recvChar) == KEY_BACKSPACE || (*recvChar) == ' ' ||
         (*recvChar) == KEY_TAB || (*recvChar) == KEY_BACKTICK ||
         (*recvChar) == KEY_ENT) // or esc / Tab
  {
    if ((*recvChar) == KEY_BACKSPACE) {
      ...
      if (numDig((*time_out_num)) >
          numDig(time_out[(*time_cyc)])) { // if previous num is bigger
        for (int i = 0;
             i < (numDig((*time_out_num)) - numDig(time_out[(*time_cyc)]));
             i++) { // delete the amount of spaces needed to cover the
                    // difference
          wmove(timeWin, 0, numDig((*time_out_num)) - 1);
          wdelch(timeWin);
        }

      } else if (numDig((*time_out_num)) <
                 numDig(time_out[(*time_cyc)])) { // if old number is smaller
        for (int i = 0;
             i < (numDig(time_out[(*time_cyc)]) - numDig((*time_out_num)));
             i++) { // insert the amount of spaces needed to cover the
                    // difference
      ...
      // change stats - free, create new one and then change it ((&stats)memcpy
      // a new address to here
      ...
      wmove(langWin, 0,
            strlen(languages[old_lang_cyc]) - 1); // end of the languege word
      lang_diff = strlen(languages[old_lang_cyc]) -
                  strlen(languages[(*lang_cyc)]); // signed
      ...
      char *ext = ".yaml";
      char *filep = malloc(homedi_len + strlen(dir) +
                           strlen(languages[(*lang_cyc)]) + strlen(ext) + 1);
      ...
      cyaml_err_t stats_err = cyaml_load_file(
          filep, &config, &stats_file_schema_top, (cyaml_data_t **)&temp, NULL);
      if (stats_err != CYAML_OK) {
        printw("we cannot load stats file for some reason!");
        getch();
        endwin();
      ...
      memcpy(address_to_stats, &temp,
             sizeof(struct stats_file *)); // changing the stats variable of the
                                           // above function
      stats = temp; // changing the local variable to point to the newly created
                    // schema bbin

      strings = restart_text(
          timeWin, langWin, (*time_out_num), languages, (*time_cyc), printed_ll,
          wordAmount, strings, line_number, existing_space, text_line_length,
          textWin, awaiting_words,
          languages[(*lang_cyc)]); // update strings after freeing it!

    } else if ((*recvChar) == KEY_TAB) {

      strings = restart_text(timeWin, langWin, (*time_out_num), languages,
                             (*time_cyc), printed_ll, wordAmount, strings,
                             line_number, existing_space, text_line_length,
                             textWin, awaiting_words, languages[(*lang_cyc)]);
...
restart_text(WINDOW *timeWin, WINDOW *langWin, long time_out_num,
  ...
  struct _word *linked_ll_initial =
      linked_ll; // through the free loop, linked_ll will become the last
                 // linked_ll element. linked_ll_initial preserves the starting
                 // point.
  // linked_ll initial should be the same as the general printed_ll; if not ->
  // meaning i'm modifying what printed_ll points to
  for (size_t i = 0; i < (*wordAmount);
       i++) { // move through all structs and free all of them and their
              // components
    tmp = linked_ll;
  ...
  char *ext = ".yaml";
  char *file =
      malloc(homedi_len + strlen(dir) + strlen(lang) + strlen(ext) + 1);
  ...
  err = cyaml_load_file(file, &config, &strings_schema_top,
                        (cyaml_data_t **)&strings,
                        NULL); // load file onto the struct strings
  if (err != CYAML_OK) {
  ...
  cyaml_save_file(
      file, &config, &strings_schema_top, strings,
      0); // after shuffle, save the shuffled words to file. that way each time
          // he shuffles he will have a random list to previous ones.
  if (err != CYAML_OK) {
  ...
  (*awaiting_words) = 0;
  init_text(linked_ll_initial, wordAmount, strings, line_number, existing_space,
            textWin, awaiting_words);
  ...
  if (line_number == 2) {
    ...
    if (existing_space[line_number] < next_word_length) {
      ...
      while (true) {
        ...
        wrap_delch(win, existing_space, line_number + 1, text_line_length,
                   awaiting_words, strings, 0);
...
void wrap_insch(WINDOW *win, int existing_space[], int line_number,
                int text_line_length, unsigned int *awaiting_words,
                char recvChar, int in_y, int in_x) {
  int text_line_index = text_line_length - 1;
  int y, x;
  bool done = false;
  while (!done) {
    if (existing_space[line_number] >
        0) { // no space means that there is no space beyond the last ' '.
             // BECAUSE, ' ' is included in the last word.
      ...
      while (true) {
        ...
        if (space_occ == true &&
            charholder_0 ==
                ' ') { // we encountered a space before, and encountered one now
                       // (new hold), stop. loop is completed.
          mvwinsch(win, in_y, in_x, recvChar);
          existing_space[line_number]--;
          done = true;
          break;
        ...
        existing_space[line_number]++;
        wmove(win, line_number + 1, 0);
        wrap_insch(win, existing_space, line_number + 1, text_line_length,
                   awaiting_words, charholder_0, line_number + 1, 0);
...
unsigned start_test_loop(WINDOW *timeWin, WINDOW *langWin, int *recvChar,
                         float *spaces, float *mistyped_spaces, int textstarty,
                         int textstartx, long time_out_num, unsigned time_cyc,
                         WINDOW *testwin, WINDOW *textWin,
                         struct _word *dynamic_ll, struct _word *printed_ll,
                         struct _strings *strings, unsigned int *wordAmount,
                         int line_number, int *existing_space,
                         int text_line_length, unsigned int *awaiting_words,
                         char **languages, struct _word **dynamic_ll_ptr) {

  /* The actual loop that runs after the user started typing */
  /* This will prompt for keys, compare them to what it should be, and then
   * print the response -> if it's good it will be painted green, else it will
   * be green */
  ...
  while ((time(NULL) - start) < time_out_num) {
    ...
    if (winch(testwin) != ' ') { // if there's a number that shouldn't be there
                                 // (from previous time counter) delch it.
      wdelch(testwin);
    }
    ...
    if ((*recvChar) == KEY_BACKSPACE) {
      ...
      if (relCursor == 0) { // backspaces a space
        ...
        if (dynamic_ll->overword) { // word is overworded
        ...
        } else { // discover underworded position (if exists)
          for (unsigned char i = 0; i < dynamic_ll->length; i++) {
            if (dynamic_ll->ver_arr[i] == NONTYPED_CHAR) {
              ...
              (*mistyped_spaces)--;
              (*spaces)++; // a space wasn't written to him, so he shouldn't
                           // lose one.
              break;
            }
          }
        ...
        wmove(textWin, row, col - 1);
        wrap_delch(textWin, existing_space, line_number, text_line_length,
                   awaiting_words, strings, col - 1);
      ...
      if (relCursor <
          dynamic_ll->length) { // he spaces in the middle of the word
        memset((dynamic_ll->ver_arr + relCursor), NONTYPED_CHAR,
               (dynamic_ll->length -
                relCursor)); // tab to the next word, non-type next chars.
        getyx(textWin, row, col);
      ...
      if (col == (text_line_length - 1 -
                  existing_space[line_number])) { // if he is on the last space,
                                                  // move to next line
        if (row == 2) {
          ...
          while (outofwords == false) {
            if (existing_space[line_number] <=
                strlen(strings->words_member[(*awaiting_words)])) {
              if (line_number == 2) {
                outofwords = true;
              }
              line_number += 1;
            ...
            if (existing_space[line_number] >
                strlen(strings->words_member[(
                    *awaiting_words)])) { // only > because if equal we can't
                                          // fill it. need the user to space to
                                          // move a line
              wprintw(textWin, "%s ", strings->words_member[(*awaiting_words)]);
              existing_space[line_number] -=
                  strlen(strings->words_member[(*awaiting_words)]) + 1;
      ...
      getyx(textWin, row, col);
      if (col == (text_line_length - 1 - existing_space[line_number]) ||
          dynamic_ll->overword == 5) {
        // if he is overwording by 5, stop insch'ing
        // if he is at the end of the line, don't allow overword
        continue;
      }
      wrap_insch(textWin, existing_space, line_number, text_line_length,
                 awaiting_words, (*recvChar), row, col);
...
struct stats_result *spawn_test(WINDOW *testwin,
  ...
  /* This initializes the test window derivatives, as well as the stats for the
   * test */
  int height = LINES / 2;
  int width = COLS / 1.5;
  ...
  WINDOW *textWin =
      derwin(testwin, 3, text_line_length, textstarty - 1,
             textstartx - 1); // create a newwin inside the current one for
                              // better positioning and wrapping
  WINDOW *timeWin = derwin(testwin, 1, switches_length, 0,
                           0); // create a newwin inside the current one for
                               // better positioning and wrapping
  WINDOW *langWin = derwin(testwin, 1, switches_length, 0,
                           35); // create a newwin inside the current one for
                                // better positioning and wrapping
  WINDOW *restartWin = derwin(
      testwin, 1, switches_length, textstarty + 7,
      (textstartx - 1) + (text_line_length) / 2 -
          (switches_length) / 2); // create a newwin inside the current one for
                                  // better positioning and wrapping
  WINDOW *togglesetWin = derwin(
      testwin, 1, switches_length, textstarty + 8,
      (textstartx - 1) + (text_line_length) / 2 -
          (switches_length) / 2); // create a newwin inside the current one for
                                  // better positioning and wrapping
  WINDOW *exitWin = derwin(testwin, 1, switches_length, textstarty + 9,
                           (textstartx - 1) + (text_line_length) / 2 -
                               (switches_length) /
                                   2); // create a newwin inside the current one
                                       // for better positioning and wrapping
  touchwin(testwin);
  wrefresh(testwin);
  wmove(textWin, 0, 0);
  ...
  cyaml_err_t err = cyaml_load_file(hd, &config, &strings_schema_top,
                                    (cyaml_data_t **)&strings,
                                    NULL); // load file onto the struct strings
  if (err != CYAML_OK) {
  ...
  shuffle(strings->words_member, strings->words_member_count);
  err = cyaml_save_file(
      hd, &config, &strings_schema_top, strings,
      0); // after shuffle, save the shuffled words to file. that way each time
          // he shuffles he will have a random list to previous ones.
  //
  if (err != CYAML_OK) {
  ...
  int existing_space[3] = {text_line_length, text_line_length,
                           text_line_length};

  unsigned int val = 0;
  unsigned int *awaiting_words = &val;

  // 3 linked lists that point to the same place, but are used in different
  // place.
  ...
  init_text(
      initializer_ll, wordAmount, strings, line_number, existing_space, textWin,
      awaiting_words); // initialize linked list, and print words on screen.
  ...
  touchwin(testwin); // only touching the father-window and then refreshing it
                     // to update all. then refreshing textWin to put the cursor
                     // back there.
  wrefresh(testwin);
  wrefresh(textWin);
  ...
  protoS = start_keys(&stats, stats, recvChar, &time_cyc, timeopts,
                      &time_out_num, time_out, timeWin, languages, &lang_cyc,
                      old_lang_cyc, lang_amount, lang_diff, langWin, printed_ll,
                      wordAmount, strings, line_number, existing_space,
                      text_line_length, testwin, textWin, awaiting_words);
  memcpy(address_to_stats, &stats,
         sizeof(struct stats_file *)); // changing the global variable, stats,
                                       // to point to the newly created schema.
  ...
  do {
    ...
    wrefresh(testwin);
    wrefresh(textWin);
    result = start_test_loop(
        timeWin, langWin, recvChar, &spaces, &mistyped_spaces, textstarty,
        textstartx, time_out_num, time_cyc, testwin, textWin, dynamic_ll,
        printed_ll, strings, wordAmount, 0, existing_space, text_line_length,
        awaiting_words, languages, &dynamic_ll);
    ...
    if (result == 0) {
      strings = restart_text(timeWin, langWin, time_out_num, languages,
                             time_cyc, printed_ll, wordAmount, strings, 0,
                             existing_space, text_line_length, textWin,
                             awaiting_words, languages[time_cyc]);
      ...
      struct _strings *protoS;
      protoS = start_keys(
          &stats, stats, recvChar, &time_cyc, timeopts, &time_out_num, time_out,
          timeWin, languages, &lang_cyc, old_lang_cyc, lang_amount, lang_diff,
          langWin, printed_ll, wordAmount, strings, line_number, existing_space,
          text_line_length, testwin, textWin, awaiting_words);
      memcpy(
          address_to_stats, &stats,
          sizeof(struct stats_file *)); // changing the global variable, stats,
                                        // to point to the newly created schema.
      if (protoS == NULL) {
        return NULL;
  ...
  for (; printed_ll != dynamic_ll->next_word;
       printed_ll = printed_ll->next_word) { // stop enumeration when getting to
                                             // word after the last
    correctWord = true;
    ...
    if (correctWord) {
      correctWordChars +=
          printed_ll->length + 1; // + 1 for correctly typed space
    }
  }

  struct stats_result *statisticsP =
      malloc(sizeof(struct stats_result)); // save to this struct the result and
                                           // return it to stats_win

  statisticsP->statistics.wpm =
      ((correctWordChars) / 5.0) * (60.0 / ((float)time_out_num));
  statisticsP->statistics.rwpm =
      (charAmount / 5) * (60 / ((float)time_out_num));
  ...
  // time_cyc is my number here, our stats located in
  // stats->times[time_cyc].best....
  if (statisticsP->statistics.wpm >= stats->times[time_cyc].best.wpm) {
    ...
    if (statisticsP->statistics.wpm == stats->times[time_cyc].best.wpm) {
      if (statisticsP->statistics.acc > stats->times[time_cyc].best.acc) {
        stats->times[time_cyc].best =
            statisticsP
                ->statistics; // if it has the same wpm and bigger acc, replace
        statisticsP->pb = true;
      }
    } else {
      stats->times[time_cyc].best =
          statisticsP->statistics; // it has bigger wpm, replace
      statisticsP->pb = true;
    }
  }

  unsigned int testsC = stats->times[time_cyc].tests_count;
  struct stats_metrics *temp =
      realloc(stats->times[time_cyc].tests,
  ...
  stats->times[time_cyc].tests[testsC] =
      statisticsP
          ->statistics; // it appends it in the local file but not in the yaml
  stats->times[time_cyc].tests_count++; // increment count

  char *dir = "/termtypetest/stats/";
  char *ext = ".yaml";
  char *file = malloc(homedi_len + strlen(dir) + strlen(languages[lang_cyc]) +
                      strlen(ext) + 1);
...
void spawn_stats(WINDOW *statswin, struct stats_result *statisticsP,
                 struct stats_file *stats, int *recvChar) {
  if (statisticsP == NULL) {
    // wprintw(statswin, "language: %s", statisticsP->language); // segmentation
    // fault, no lang...
...
time60: \n wpm: %f; raw wpm: %f; accuracy: %f\n \
time120: \n wpm: %f; raw wpm: %f; accuracy: %f\n",
            stats->times[0].best.wpm, stats->times[0].best.rwpm,
            stats->times[0].best.acc, stats->times[1].best.wpm,
            stats->times[1].best.rwpm, stats->times[1].best.acc,
            stats->times[2].best.wpm, stats->times[2].best.rwpm,
            stats->times[2].best.acc, stats->times[3].best.wpm,
            stats->times[3].best.rwpm, stats->times[3].best.acc);
  } else {
    if (statisticsP->pb) {
    ...
    wprintw(statswin, "RESULTS: \nwpm: %f\n raw wpm: %f\n accuracy: %f\n",
            statisticsP->statistics.wpm, statisticsP->statistics.rwpm,
            statisticsP->statistics.acc);
  }
